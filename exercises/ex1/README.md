...
1. Log on to SAP Data Intelligence and enter the Launchpad application. Then start the ***Modeler*** application.
   - Follow the link to your assigned Data Intelligence instance.
   - The tenant name should be ***"da281"***.
...
6.	The ABAP CDS Reader operator shell has received the operator's metadata from S/4HANA and has configured the Pipeline node (for example, the operator now has an output port). Now fill in the configuration parameters in the panel:
      ...
      - ABAP CDS Name: `Z_CDS_EPM_BUPA`, which is the "simple" CDS View that got created in the Deep Dive 1 demo
...
7.	From the operator list on the left side, drag the ***Get Header*** operator and drop it into the Pipeline canvas. Search for the ***Get Header*** operator in the search field to get access to the operator:<br><br>
...
10.	***Save*** your Pipeline.
      ...
      As Name, enter **`teched.XXXX.EPM_Customer_Replication_to_HANA`**, where **XXXX** is your user name, for example "teched.TA99.EPM_Customer_Replication_to_HANA".<br>
...
5. Enter the needed configuration parameters for the ***Write File*** operator. These are:<br>
   ...
   - Connection: Choose type ***Connection Management*** and then the connection ID **`HANA_CLOUD`**<br>
...
3.	In the configuration panel for the ABAP CDS Reader, specify the S/4HANA Connection: Select `S4_HANA`. Then click on the ***Version*** selection icon.<br><br>
![](/exercises/ex1/images/ex1-007b.JPG)<br><br>
...
6.	The ABAP CDS Reader operator shell has received the operator's metadata from S/4HANA and has configured the Pipeline node (for example, the operator now has an output port). Now fill in the configuration parameters in the panel:
      ...
      - ABAP CDS Name: `Z_CDS_EPM_SO`, which is the "more complex" CDS View that got created in the Deep Dive 1 demo
      - Transfer Mode: `Replication`
      - ***Records per Roundtrip: 5000***
      - Leave all other parameters as-is.<br><br>
   ![](/exercises/ex1/images/ex1-039b.JPG)<br><br>
   
1. From the operator list on the left side, drag and drop the ***Get Header*** operator into the Pipeline canvas. Search for the ***Get Header*** in the search field of the operator:<br><br>
	![](/exercises/ex1/images/ex1-128b.JPG)<br><br>	
	Then connect the output port of the ABAP CDS Reader with the input port of the Get Header operator by pulling the mouse pointer from one port to the other while the left mouse button is pressed.<br><br>
...
8.	From the operator list on the left side, drag the ***Wiretap*** operator and drop it into the Pipeline canvas. Then connect the output port of the ABAP CDS Reader with the input port of the Wiretab operator by pulling the mouse pointer from one port to the other while the left mouse button is pressed.<br><br>
...
12. Enter the needed configuration parameters for the ***Write File*** operator. These are:
   - Label: **`Sales Order to Data Lake`** (Optional)
...
17. For validating the correct creation of the file in the DI Data Lake, please leverage the Data Intelligence Metadata Explorer again. Go back to the Launchpad and start the Metadata Explorer application, if not already launched from the previous exercise.<br><br>
...
19. In order to easily find our connection to the target DI Data Lake Object Store, you may leverage the search functionality. Enter `DI_DATA_LAKE` into the search field and click on the spyclass icon. Click on the **`DI_DATA_LAKE`** tile.<br><br>
![](/exercises/ex1/images/ex1-051b.JPG)<br><br>

20. On the next drill-down view, click on the **`DA281`** directory that you had specified in the ***Write File*** operator, and then drill down to your specific User directory, for example **`TA99`**.<br><br>
...
**Congratulations!** You have created the Sales Order extraction from a delta-enabled, more complex ABAP CDS View into the DI Data Lake Object Store. Because you have chosen the transfer mode ***"Replication"*** in the CDS Reader operator configuration, the Pipeline has conducted the Initial Load and would now wait for any changes (inserts, updates, deletions) in the S/HANA EPM Sales Order object as long as it is running.
...
1. In SAP Data Intelligence, open the ***Modeler*** application. Make sure the scope of the Modeler UI is on tab ***Graphs*** (see left side). If not still open from the previous exercise part, search for your Sales Order Pipeline by entering your user name into the search field on top of the list of Pipelines. Click on Pipeline with the label `XXXX - Replicate S/4HANA EPM Sales  Order Data to Data Lake`, where XXXX is your user name, for example "TA99 - Replicate S/4HANA EPM Sales  Order Data to Data Lake". The Pipeline will get opened then or will get the focus on the UI if it was already opened. If you cannot see the complete label of the Pipeline, just hover with your mouse over the pipeline item in the list.<br><br>
...
3. You are prompted for some inputs. Please enter the following values.
    - As Name, enter **`teched.XXXX.EPM_SalesOrder_Replication_Enrich_to_Data_Lake`**, where **XXXX** is your user name, for example "teched.TA99.EPM_SalesOrder_Replication_Enrich_to_Data_Lake".<br>
    - As Description, please enter **`XXXX - Replicate and Enrich S/4HANA EPM Sales Order Data to Data Lake`**,where **XXXX** is your user name, for example "TA99 - Replicate and Enrich S/4HANA EPM Sales Order Data to Data Lake". (The description will show up in the Pipeline status information later on.)<br>
    - As Catergory, enter **`da281`**, which is the name under which you can find your Pipeline in the ***Graphs*** tab.<br>
    - Then click ***OK***.<br><br>
...
7. Open the configuration of the ***CDS Reader Operator*** operator and change the name of the subscription to a new name, e.g. adding a ***_Enriched*** at the end.<br><br>
![](/exercises/ex1/images/ex1-139b.JPG)<br><br>

8. Click on the ***Operators*** tab on the left side in order to get the list of operators. Find the ***Graph Terminator*** operator and drag it from the operator list into the canvas. Then connect the upper output port ("file") of the File Writer with the input port of the Graph Terminator operator.<br><br>
...
9. ***Save*** and ***Run*** the Pipeline. Wait until the Pipeline status turns from ***pending*** to ***processing*** and finally to ***completed***.<br><br>
![](/exercises/ex1/images/ex1-062b.JPG)<br><br>

10. After the completion of the Pipeline execution, **remove the Graph Terminator** operator from the Pipeline again.<br><br>
...
13. Then click the ***DI Data Lake Source File*** folder icon to browse through the DI Data Lake bucket. Drill down to your individual folder under ***DA281*** and select the file ***Sales_Order_Staging.csv***. Then click ***OK***. <br><br>
...
15. Now connect the **upper output port ("file") of the File Writer** operator with the **input port of the Structured File Consumer** operator. The ***Structured File Consumer*** operator takes the signal on the input port just as a trigger for commencing its logic. It's an optional input but our approach ensures that the operator is only executed after the file from the previous node has been successfully written on the DI Data Lake. (If the input port of a Structured File Consumer is not connected, the operator will start with the Pipeline execution.)<br><br>
...
16. Now we need to a ***Table Consumer*** operator for the Customer master data extraction from HANA Cloud. Go to the operator tab on the left-hand panel to drag and drop the Table Consumer operator in your graph.<br><br>
...
17. On this new operator, open the configuration panel and click the **DI Data Lake Source File** folder icon to browse through the DI Data Lake bucket. Drill down to your individual folder under **DA281** and select the file **`Customer_Master.csv`**. Then click ***OK***..<br><br>
![](/exercises/ex1/images/ex1-075b.JPG)<br><br>

	Select the service to “HANA_DB” and choose “HANA_CLOUD” as Connection ID, which has been used as target for storing the Customer Master Data in our previous exercise. <br><br>
...
18.  Once you selected the Customer data in HANA, the dialog will automatically fetch the metadata of the table and show on the bottom of the screen: <br><br> 
...
20. Double-click on the ***Data Transform*** operator.<br><br>
![](/exercises/ex1/images/ex1-081b.JPG)<br><br>

21. In the details view, from the list of Data Operations on the left side, drag the ***Join*** operation into the canvas with the existing two ***input nodes*** and connect these with the ***Join*** operation.<br><br>
![](/exercises/ex1/images/ex1-082b.JPG)<br><br>

22. Double-click on the ***Join*** operation.<br><br>
![](/exercises/ex1/images/ex1-083b.JPG)<br><br>

23. Re-arrange the position of input tables on the canvas as per your convenience.<br><br>
![](/exercises/ex1/images/ex1-084b.JPG)<br><br>

24. Connect ***"BUYERGUID" field of Join_Input1*** (Sales Order) with the ***"NODEKEY" field of Join_Input2*** (Customer). This will map the Customer GUID fields of the two tables. We're using an INNER JOIN..<br><br>
...
25. Proceed to tab ***Columns*** in order to select the output fields. Click on the ***Auto map by name*** icon (see red box), which enter all fields of the two input nodes to the list of output fields.<br><br> 
![](/exercises/ex1/images/ex1-087b.JPG)<br><br>

26. Now you can choose if you want to remove some columns in the target data set, e.g. in this case we are selecting & removing the following two columns as an example:<br><br>
![](/exercises/ex1/images/ex1-088b.JPG)<br><br>

27. Navigate back.<br><br>
![](/exercises/ex1/images/ex1-089b.JPG)<br><br>

28. Click with the right mouse button on the output port of the ***Join*** operation. Then click on ***Create Data target***.<br><br>
![](/exercises/ex1/images/ex1-090b.JPG)<br><br>

29. A new output node has been created, which will now be available on the Data Transfer operator. Navigate further back.<br><br>
![](/exercises/ex1/images/ex1-091b.JPG)<br><br>

30. Back on the Pipeline canvas, drag the ***Structured File Producer*** operator from the list of operators into the Pipeline canvas. Then connect the ***output port of the Data Transform*** with the ***input port of the Structured File Producer***.<br><br>
![](/exercises/ex1/images/ex1-092b.JPG)<br><br>

31. Open the configuration panel of the ***Structured File Producer*** operator aby clicking on the Edit / Pencil icon of the Target Configuration to start the configuration of your target DI Data Lake scenario:<br>
	...
	Browse to the path in DI Data Lake with /shared/DA281/XXXX, where XXXX stands for your user ID e.g. /shared/DA281/TA99 and click on + icon: <br><br>
...
32. Open the configuration of the ***CDS Reader Operator*** operator and change the name of the subscription to a new name, e.g. adding a ***_Enriched_Delta*** at the end. This is required as a new initialization inlcuding the initial load is requiring a new subscription on the SAP S/4HANA side:<br><br>
![](/exercises/ex1/images/ex1-140b.JPG)<br><br>
...
Please continue to [Exercise 1.5 - Using a custom ABAP Operator to verify your Delta Replication of EPM Sales Orders](../../exercises/ex2/Readme.md).
