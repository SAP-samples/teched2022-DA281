...
In this section we will demonstrate how to create a custom ABAP operator in SAP S/4HANA and trigger its execution from a Pipeline in SAP Data Intelligence.<br>
The ABAP operator that we will be implementing provides a feature which is needed by some of the other Deep Dive sessions and Exercises of this workshop when it comes to extracting and replicating delta information from an ABAP source system. We will realize the functionality for triggering the execution of an ABAP program in the source system that generates Sales Order data for the Enterprise Procurement Model (EPM).<br>
